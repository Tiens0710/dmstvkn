...
const ShopManagementApp: React.FC = () => {
    ...
    const [overBudgetCategories, setOverBudgetCategories] = useState<number>(0);
    ...
    const loadDashboardData = useCallback(async () => {
        try {
            ...
            const txnCount = await getTransactionCountToday();
            const overCount = await getOverBudgetCount();
            setMonthlyExpense(expense);
            setMonthlyIncome(income);
            setTodayTransactions(txnCount);
            setOverBudgetCategories(overCount);
        } catch (error) {
    ...
    const handleFeaturePress = (feature: Feature, index: number): void => {
        ...
        if (feature.route === 'Transactions') {
            const type = feature.id === 1 ? 'expense' : feature.id === 2 ? 'income' : undefined;
            (navigation as any).navigate('Transactions', { type });
        } else {
    ...
    useEffect(() => {
        const listenerId = revenueAnim.addListener(({ value }) => {
            setAnimatedRevenueValue(Math.floor(value * monthlyExpense));
        });
    ...
    const mainFeatures: Feature[] = [
        {
            id: 1,
            title: 'Chi tiêu',
            subtitle: 'Ghi & theo dõi chi tiêu',
            icon: 'money-off',
            color: '#DC2626',
            bgColor: '#FFFFFF',
            route: 'Transactions'
        ...
        <View style={styles.container}>
            ...
            <Animated.View style={[
                ...
                >
                    ...
                    {/* Monthly Expense Overview Card - Floating over header bottom */}
                    <View style={styles.revenueCard}>
                        <View style={styles.revenueHeader}>
                            <View>
                                <Text style={styles.revenueTitle}>Chi tiêu tháng này</Text>
                                <Text style={styles.revenueSubtitle}>
                                    {todayTransactions} giao dịch hôm nay • {formatTime(currentTime)}
                                </Text>
                            </View>
                            <View style={styles.revenueBadge}>
                                <Text style={styles.revenueBadgeText}>↗ +8%</Text>
                            </View>
                        </View>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Icon name="arrow-downward" size={14} color="#34D399" style={{ marginRight: 4 }} />
                                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11 }}>Thu nhập  </Text>
                                <Text style={{ color: '#34D399', fontSize: 12, fontWeight: '600' }}>{formatCurrency(monthlyIncome)}</Text>
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Icon name="arrow-upward" size={14} color="#F87171" style={{ marginRight: 4 }} />
                                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11 }}>Còn lại  </Text>
                                <Text style={{ color: '#F87171', fontSize: 12, fontWeight: '600' }}>{formatCurrency(monthlyIncome - monthlyExpense)}</Text>
                            </View>
                        </View>
                        <TouchableOpacity
                    ...
                    <View style={styles.quickActionsContainer}>
                        <TouchableOpacity
                            ...
                            onPress={() => safeNavigate('LinkAccount')}
                        >
                            <Text style={styles.connectBankText}>Liên kết tài khoản ngân hàng</Text>
                            <Icon name="chevron-right" size={16} color="#4F46E5" />
                        </TouchableOpacity>

                        <TouchableOpacity
                            style={styles.addProductBtn}
                            onPress={() => safeNavigate('AddTransaction')}
                        >
                            <Animated.View style={{
                            ...
                            <Text style={styles.addProductText}>Ghi chi tiêu</Text>
                        </TouchableOpacity>
                    </View>

                    {/* Main Features Grid - Expense Management */}
                    ...
                    {/* Budget Alert */}
                    <TouchableOpacity onPress={() => safeNavigate('Budget')}>
                        <Animated.View style={[
                            styles.alertSection,
                            {
                            ...
                            <Text style={styles.alertText}>
                                {overBudgetCategories} danh mục đã vượt ngân sách tháng này
                            </Text>
                        </Animated.View>
                    </TouchableOpacity>
                    ...
                    <View style={styles.servicesSection}>
                        <Text style={styles.sectionTitle}>Công cụ tài chính</Text>
